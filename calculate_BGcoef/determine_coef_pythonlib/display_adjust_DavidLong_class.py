...
class display_adjust_DavidLong_class:
    ...
    def display_compare_gama_nadir(self):
        print('hello')
        path='/mars/jzhou/data/BG_AdaptiveWindow_output/Remap_ATMS__Ch1_5p2_Ch1_3p3/BGcoef/window_out0p01/'\
             +'WindowAdjust_noiselv2.5_cycleSearch/atmch0/fov47'
        fname='dBs_winsize_Q0_e2_gm.pickle'
        with open(path+'/dBs_winsize_Q0_e2_gm.pickle','rb') as f:
            [dBs,winsize_allwin,Q0_allwin,e2_allwin,gm_allwin,
             alpha_allwin,sumalpha_allwin,cond_S_allwin]=pickle.load(f)
        print(gm_allwin)

    def display_compare_gama(self):
        path='/mars/jzhou/data/BG_AdaptiveWindow_output/Remap_ATMS__Ch1_5p2_Ch1_3p3/BGcoef/window_out0p01/'\
             +'WindowAdjust_noiselv2.5_cycleSearch/atmch0/' #gama vary along Fov to ensure noiselevel=2.5
       
        ##path='/mars/jzhou/data/BG_AdaptiveWindow_output/Remap_ATMS__Ch1_5p2_Ch1_3p3/BGcoef/window_out0p01/'\
        ##     +'WindowAdjust_noiselv999_singleSearch/atmch0/' #gama fixed to mean of above

        #path='/mars/jzhou/data/BG_AdaptiveWindow_output/Remap_ATMS__Ch1_5p2_Ch1_3p3/BGcoef/window_out0p01/'\
        #     +'WindowAdjust_noiselv999_singleSearch_gmMin/atmch0/' #gama fixed to mean of above

        gm=[]
        noise_amply=[]
        alpha=[]
        for ifv in range(96):
            with open(path+'fov'+str(ifv)+'/dBs_winsize_Q0_e2_gm.pickle','rb') as f:
                [dBs,winsize_allwin,Q0_allwin,e2_allwin,gm_allwin,
                 alpha_allwin,sumalpha_allwin,cond_S_allwin]=pickle.load(f)
            dBs=np.array(dBs)
            gm_allwin=np.array(gm_allwin)
            e2_allwin=np.array(e2_allwin)
            alpha_allwin=np.array(alpha_allwin)
            idx=np.where(dBs==-5)
            gm.append(gm_allwin[idx][0])
            noise_amply.append(e2_allwin[idx][0])
            alpha.append(alpha_allwin[idx][0])
            print(ifv,alpha_allwin[idx][0])
        print('gm range: ',np.min(gm),np.max(gm),np.mean(gm))
        print('noise range: ',np.min(noise_amply),np.max(noise_amply))
        print(len(alpha[1]),len(alpha[47]))
        with open(path+'gama_vary.pickle','wb') as f:
            pickle.dump([gm,noise_amply,alpha],f)
    ...
    def display_compare_winsize(self):
        ...
        if task=='Remap_ATMS_5p2_3p3':
            ...
            wins=['out0']
            dBs=list(np.arange(-17,0,2))

            input_dir='/mars/jzhou/data/BG_AdaptiveWindow_output/Remap_ATMS__Ch1_5p2_Ch1_3p3_2p5bw/BGcoef/'
            com_dir='/WindowAdjust_noiselv2.5_cycleSearch/atmch0/'
            ...
            for win in wins:
                for dB in dBs:
                    if win!='out0' and dB!=-999: continue
                    nwin+=1
                    winsiz_all.append([])
                    labels.append(win+str(dB))
                    for fov in range(96):
                        fname=input_dir+'window_'+win+com_dir+'fov'+str(fov)+'/Window_'+str(dB).replace('-','neg')+'dB.pickle' 
                        print(fname)
                        with open(fname,'rb') as f:
                            [windowsize,windowindx,idx_src,S,cond_num]=pickle.load(f)
                        winsiz_all[nwin-1].append(windowsize)

            save_dir=input_dir+'comp_winsize/'
    ...
    def display_window(self,dB):
        ...
        with open(os.path.join(self.winadj_subfigdir+'Window_fov'+str(self.ifr)+'_win'+str(winsize)+'_'+dB_str+'_paper.pickle'),'wb') as f:
            pickle.dump([minlon,maxlon,minlat,maxlat,
                         self.tgt_edg_lon,self.tgt_edg_lat,
                         lon,lat],f)
        print('finish: '+os.path.join(self.winadj_subfigdir+'Window_fov'+str(self.ifr)+'_win'+str(winsize)+'_'+dB_str+'_paper.pickle'))
    ...
    def display_alpha(self,dB):
        ...
        ax.set_xlabel('window index '+str(np.sum(alpha)))
        ax.set_ylabel('alpha')
    ...
    def display_G(self,tab):
        print(self.winadj_subdir+tab+'.pickle')
        with open(self.winadj_subdir+tab+'.pickle','rb') as f:
            [antwgt,trans,antpos_nx,antpos_ny]=pickle.load(f)

        #restrict=0.031  #-15dB for src
        restrict=0.02  #-15dB for src
        # record the idd of src, will be applied to tgt and sim---
        if tab[0:4]=='Gsrc': 
            self.idd_zero=np.where(antwgt<=restrict) # the area where antegt<=restrict will set to zero, remove the glitch in the brink
            self.idd=np.where(antwgt>restrict)  # the square area will be adopted

        # set the area where antwgt<=restrict to zero
        antwgt[self.idd_zero]=0
        # adopt the square area
        z=antwgt[min(self.idd[0]):max(self.idd[0])+1,min(self.idd[1]):max(self.idd[1])+1]
        tr=trans[min(self.idd[0]):max(self.idd[0])+1,min(self.idd[1]):max(self.idd[1])+1]
        x=antpos_nx[min(self.idd[0]):max(self.idd[0])+1,min(self.idd[1]):max(self.idd[1])+1]*self.delx
        y=antpos_ny[min(self.idd[0]):max(self.idd[0])+1,min(self.idd[1]):max(self.idd[1])+1]*self.dely

        ## normalize and centerize 
        # way 1: determine center and max by the mean of the z that larger than 0.9
        #        seems a better way, modify the parameter 0.9 to fit specific case
        idd=np.where(z>np.max(z)*0.98)
        print('num of points to determine max',len(idd[0]))
        x=x-np.mean(x[idd])
        y=y-np.mean(y[idd])
        z=z/np.mean(z[idd])
        ...
        freqx=np.fft.fftfreq(nx,d=self.delx)
        freqy=np.fft.fftfreq(ny,d=self.dely)
        freqx=freqx[np.argsort(freqx)]
        freqy=freqy[np.argsort(freqy)]

        #Shift the zero-frequency component to the center of the spectrum.
        fz=np.fft.fftshift(np.abs(np.fft.fft2(z,norm="ortho")))
        fz=fz/np.max(fz)
 
        # way 2: determine center and max by Gauss fit function, not good
        #idd=np.argmax(z)
        #z=z/z[idd]
        #x=x-x[idd]
        #y=y-y[idd]
        ...
        if tab[0:4]=='Gsrc': self.area=[np.min(x),np.max(x),np.min(y),np.max(y)]
        ...
        ax=fig.add_subplot(211)
        #z[self.idd_zero]=np.nan

        #print(z.shape)

        im=ax.imshow(z.T,cmap='jet',origin='lower',
                     extent=[xmin,xmax,ymin,ymax],
                     vmin=0, vmax=1)
        if n_bm>5:
            ax.scatter(x_bm,y_bm,s=20,marker='X',c='k')
            if self.ifr==47: ax.plot(x_cr,y_cr,'k-')
        ...
        ax=fig.add_subplot(212)
        im=ax.imshow(fz.T,cmap='jet',origin='lower',
                               extent=[freqx[0],freqx[-1],freqy[0],freqy[-1]],
                               vmin=0, vmax=1)
        ...
        #im=ax.imshow(tr.T,cmap='jet',origin='lower',
        #             extent=[xmin,xmax,ymin,ymax],
        #             vmin=np.min(tr),vmax=np.max(tr))
        #ax.set_ylabel('tr')
        #divider=make_axes_locatable(ax)
        #cax=divider.append_axes('right',     #position
        #                        size='5%',   #width
        ...
        with open(self.winadj_subfigdir+tab+'_paper.pickle','wb') as f:
            pickle.dump([[x,y,z],[xmin,xmax,ymin,ymax],[x_bm,y_bm],[x_cr,y_cr],
                         fz,[freqx[0],freqx[-1],freqy[0],freqy[-1]]],f)
    ...
    def display_compare_Q0e2(self):
        ...
        if task=='Remap_ATMS_5p2_3p3':
            ...
            input_dir='/mars/jzhou/data/BG_AdaptiveWindow_output/Remap_ATMS__Ch1_5p2_Ch1_3p3/BGcoef/'
